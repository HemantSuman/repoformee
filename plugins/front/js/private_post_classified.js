...
//    $('.parent_cat').each(function (index, value) {
    $('.parent_cat').removeClass('selected');
//    });
    ...
    $.each(categoryJson[categoryId][0]['children'], function (index, val) {
        console.log(val);
        var li = "<li><a class='child_cat' p_id='" + val.pid + "' categoryId='" + val.id + "' href='javascript:void(0);'>";
        li += "<span class='cat-img'><img class='cat-icon' src='" + root_url + "/upload_images/categories/icon/" + val.id + "/" + val.icon + "' >";
        li += "</span><span class='cat-name'>" + val.text + "</span></a></li>";
...
$(document).on('click', '.child_cat', function () {
    ...
    $('.child_cat').removeClass('selected');
    $(this).addClass('selected');
...
//                console.log(response.childCatDetail.is_sellable)
                if (response.childCatDetail.is_sellable == 0 && !response.template_arr) {
                    ...
                    if (response.template_arr != null && response.template_arr.is_inspection_date == 1) {
                        ...
                        html_content += "<li class='no-ad-padding'><input is_required='0' iderr='inspection_date' inspection_date='1' class='datepicker inspection_date inspection_date_1 textRequired' type='text' name='other_value[is_inspection_date][1][title]'></li>";
                        html_content += "<li class='no-ad-padding'><input is_required='0' iderr='inspection_date' inspection_date='2' class='datepicker inspection_date inspection_date_2 textRequired' type='text' name='other_value[is_inspection_date][2][title]'></li>";
                        html_content += "<li class='no-ad-padding'><input is_required='0' iderr='inspection_date' inspection_date='3' class='datepicker inspection_date inspection_date_3 textRequired' type='text' name='other_value[is_inspection_date][3][title]'></li>";
...
$(document).on('change', '.shipping_check', function () {
    ...
    if ($(thisObj).val() == 1) {
        var html_content = "";
        html_content += "<input is_required='1' iderr='ship_amount' class='textRequired' type='text' name='ship_amount' placeholder='Price'>";
        html_content += "<p class='ad-error' id='ship_amount_error'></p>";
        html_content += "<span class='ad-form-price'>AUD</span>";
...
$(document).on('click', '.planSelectBtn', function () {
    ...
    $('.is_premium_sub_cat').val(0);
    $('.is_premium_parent_cat').val(0);
    $('.featured_classified').val(0);
    $('.is_premium').val(0);
    
    var packagesjson = JSON.parse($('.packagesjson').text());
    
    if(packagesjson[planId][0]['classified_type'] == 'premium_child'){
        $('.is_premium_sub_cat').val(1);
        $('.is_premium').val(1);
    } else if(packagesjson[planId][0]['classified_type'] == 'premium_parent'){
        $('.is_premium_parent_cat').val(1);
        $('.is_premium').val(1);
    } else if(packagesjson[planId][0]['classified_type'] == 'featured'){
        $('.featured_classified').val(1);
    }
    
    $('#package_classified').fadeOut(500, function () {
...
$(document).on('click', '.finalSubmitAfterPrev', function () {
    ...
    $.ajax({
        url: root_url + '/user/classifieds/final_submit_post_private',
        method: "POST", // Type of request to be send, called as method
        data: {
            "_token": $('meta[name="csrf-token"]').attr('content'),
            'id': $('.class_id').val(),
...
$(document).on('click', '.back_content5', function () {
